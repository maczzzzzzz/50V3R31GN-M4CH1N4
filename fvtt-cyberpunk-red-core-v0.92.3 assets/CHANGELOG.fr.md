...
Nous avons déménagé le serveur Discord ! If you need any help with this release, want to help with future releases, or just want to join our community we can now be found in our new [Discord server](https://discord.gg/TsvcZUEtbJ). Nous espérons vous y voir !
...
Les effets avec les modificateurs de situation ont reçu les réglages appropriés sur tous les éléments du compendium, mais vous devrez les mettre à jour manuellement sur les éléments qui existent déjà sur les acteurs. See New Features -> Roll Modifiers section of Changelog for more details.
...
Nous avons ajouté les fonctionnalités pour les thèmes spécifiques au système. Cela nous permet d'expédier quelques thèmes par défaut (mode sombre!). You can configure this in the `Settings > Cyberpunk RED - CORE` section.
