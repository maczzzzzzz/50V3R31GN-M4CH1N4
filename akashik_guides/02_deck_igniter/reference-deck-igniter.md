...
## IMMERSIVE HACKING
The v3.0.0 release includes **AI-Driven Script Injection**. The Director (Node B) can now inject raw JavaScript into the Foundry client to simulate "Netrunner Attacks":
...
These effects are managed by the `AntagonistService` and are verified via the **Synthetic Gauntlet** automated audit.
