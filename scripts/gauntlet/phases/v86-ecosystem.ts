...
export const phaseV86Ecosystem: GauntletTask = {
  ...
  async audit(ctx): Promise<AuditResult> {
    ...
    const checkObsidian = await ctx.shell('ls sidecars/sidecar-obsidian-plugin/packages/hermes-core/src/main.ts');
    if (checkObsidian.code === 0) results.push('● Native Obsidian Bridge materialized.');
    
    const checkLogseq = await ctx.shell('ls sidecars/sidecar-logseq-plugin/packages/hermes-core/src/main.js');
