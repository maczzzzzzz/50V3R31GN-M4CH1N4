...
**Objective:** Carefully roll out isolated appendages and optimize mesh responsiveness.
**Status:** ✅ MATERIALIZED

*   [x] **Hierarchy of Responsiveness:** Materialized two-tier inference mesh (Node B/D split).
...
## ◈ EMERGENCY REMEDIATION: MESH STABILIZATION (ACTIVE)
**Objective:** Resolve systemic mesh failures by transitioning to a Hybrid Sovereign Architecture (NixOS Stable + Docker Compose).
**Status:** ⚡ IN PROGRESS (2026-05-11)

*   [ ] Task 1: **NixOS 24.11 Standardization** - Pin all nodes to stable Nixpkgs and enable OCI/Docker modules.
*   [x] Task 2: **WSL2 Bridge Fix (Node B)** - ❌ FAILED. `dxg Ioctl -22` persists despite BIOS changes. CPU fallback active.
*   [ ] Task 3: **Heterogeneous OCI Materialization** - Deploy `docker-compose.yml` for NVIDIA (A/C), AMD (B), and Intel NPU (D).
*   [x] Task 4: **LiteLLM Central Hub** - Refactor Node B Proxy to Host Network Mode. Use local IP `10.0.0.13` for Node D due to Tailscale port 8080 blockage.
*   [ ] Task 5: **Node D NPU/iGPU Acceleration** - Enable `openvino` or `oneapi` for Meteor Lake hardware to remediate 0.4 t/s CPU bottleneck.
*   [ ] Task 6: **Node B Windows Bridge** - Host `llama-server.exe` on Windows and proxy via LiteLLM to bypass WSL2 bridge faults.

## ◈ EMERGENCY REMEDIATION: PERFORMANCE PATCH (TURBOQUANT)
**Objective:** Inject TurboQuant (3-bit KV-cache) across the stable mesh to multiply effective context.
**Status:** ⏳ QUEUED

*   [ ] Task 1: **ik_llama Containerization** - Build vendor-specific OCI images for `ik_llama.cpp` with TQ support.
*   [ ] Task 2: **Node B Acceleration** - Enable `vllm-rocm` + `turboquant` plugin.
*   [ ] Task 3: **Node D Acceleration** - Enable `openvino` + `turbo3` NPU kernels.
