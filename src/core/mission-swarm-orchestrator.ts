...
export class MissionSwarmOrchestrator {
  ...
  async generateMission(district: string): Promise<MissionBlueprint> {
    ...
    const brief = await this.config.ollama.generateNarrative(briefPrompt, district);
    ...
    const response = await this.config.ollama.generateNarrative(tacticalPrompt, brief);
