...
export class TurnDaemon {
  ...
  async runTurn(sensoryContext: string): Promise<TurnResult> {
    const startMs = Date.now();

    // Stage 1: Reason
    const reasoning = await this.stageReason(sensoryContext);

    // Stage 2: Intent
    const intent = await this.stageIntent(reasoning);

    // Stage 3: Action (hard timeout + Zod validation)
    const action = await this.stageAction(intent, reasoning, sensoryContext);

    // Stage 4: Validate with Node A
    const { action: finalAction, validated } = await this.stageValidate(action);
  ...
  private async stageReason(sensoryContext: string): Promise<string> {
    const logs: NpcLog[] = this.lifePathService.getRecentLogs(this.npcId, 5);
    ...
    const reasonPrompt =
      `You are an NPC with id "${this.npcId}" in a Cyberpunk RED tactical scenario.\n` +
  ...
  private async stageAction(
    ...
    try {
      ...
      this.warn(`Stage 3 timed out or threw: ${reason}. Falling back to idle.`);
    ...
    if (!jsonMatch) {
      this.warn('Stage 3: no JSON object found in LLM response. Falling back to idle.');
    ...
    try {
      ...
      this.warn('Stage 3: JSON.parse failed. Falling back to idle.');
      return idleAction('parse_failure');
    }

    const result = NpcActionSchema.safeParse(parsed);
    if (!result.success) {
      this.warn(`Stage 3: Zod validation failed — ${result.error.issues[0]?.message ?? 'unknown'}. Falling back to idle.`);
  ...
  private async stageValidate(
    ...
    try {
      rpcResult = await this.clawlink.executeRpc<{ valid: boolean; reason?: string }>(
        'validate_npc_action',
        { npcId: this.npcId, action },
      );
    } catch (err) {
      // Node A unreachable — fail-open
      const msg = err instanceof Error ? err.message : String(err);
      this.warn(`Stage 4: Node A unreachable (${msg}). Allowing action through (fail-open).`);
      return { action, validated: false };
    }

    if (!rpcResult.valid) {
      const fallbackReason = rpcResult.reason ?? 'rejected_by_rules_vault';
      this.warn(`Stage 4: Node A rejected action — "${fallbackReason}". Falling back to idle.`);
