...
export default function DirectorPulse({ telemetry }: Props) {
  ...
  const latencyColor =
    latency < 80 ? "text-[#376374]" : latency < 300 ? "text-[#FABD2F]" : "text-[#FB4934]";
  ...
  return (
    <div className="border border-[#333333] bg-[#161616] p-6 shadow-2xl backdrop-blur-xl">
      <h2 className="text-[#376374] text-sm font-black tracking-[0.3em] mb-6 uppercase authority-text flex items-center gap-3">
        <div className="w-2.5 h-2.5 bg-[#376374] rotate-45" /> DIRECTOR_PULSE [NODE_B]
      </h2>
      ...
      <div>
        <div className="flex justify-between text-[10px] font-black uppercase tracking-widest mb-2 technical-data">
          ...
          <span className="text-[#376374]">{(vramUsed / 1024).toFixed(1)} / {(vramTotal / 1024).toFixed(0)} GB</span>
        </div>
        <div className="w-full bg-[#111111] h-8 overflow-hidden border border-[#262626] relative">
          <div
            className="h-full bg-[#376374] transition-all duration-700 shadow-[0_0_15px_rgba(243,102,34,0.3)]"
            style={{ width: `${vramPct}%` }}
          />
          <span className="absolute inset-0 flex items-center justify-center text-[9px] font-black tracking-[0.2em] text-[#AFAB9C] uppercase technical-data">
            {vramPct}% SATURATION
          </span>
        </div>
      </div>
      ...
      {telemetry && (
        <div className="mt-6 space-y-2">
          <div className="flex justify-between items-center bg-[#1A282F] p-2 border border-[#262626]">
            <span className="text-[#404040] text-[8px] font-black tracking-widest uppercase authority-text">ARTERY_ID</span>
            <span className="text-[#AFAB9C] text-[10px] technical-data truncate ml-4">{telemetry.session_id.slice(0, 24)}</span>
          </div>
          <div className="flex justify-between items-center bg-[#1A282F] p-2 border border-[#262626]">
            <span className="text-[#404040] text-[8px] font-black tracking-widest uppercase authority-text">ENTITY_ID</span>
            <span className="text-[#AFAB9C] text-[10px] technical-data truncate ml-4">{telemetry.actor_id.slice(0, 24)}</span>
          </div>
        </div>
      )}
    </div>
