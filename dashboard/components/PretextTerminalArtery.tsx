...
export default function PretextTerminalArtery() {
  ...
  const getSeverityColor = (sev: string) => {
    switch (sev) {
      ...
      case 'VETO': return 'text-[#376374] font-black shadow-[0_0_10px_#376374]';
      case 'AUDIT': return 'text-[#836A46] italic';
      default: return 'text-[#B8BB26]';
    }
  };

  return (
    <div ref={scrollRef} className="flex flex-col h-full font-sans text-[9px] bg-[#050505] p-3 overflow-y-auto selection:bg-[#376374] selection:text-black">
      {logs.map((entry, i) => (
        <div key={i} className="flex gap-3 py-1 opacity-70 hover:opacity-100 transition-opacity border-b border-[#161616] last:border-none technical-data">
          ...
          <span className="text-[#AFAB9C] break-all leading-tight">{entry.message}</span>
        </div>
      ))}
      {logs.length === 0 && (
        <div className="text-[#404040] font-black tracking-widest p-4 animate-pulse uppercase">Waiting for artery pulse...</div>
      )}
      <div className="w-1.5 h-3 bg-[#376374] animate-pulse mt-2" />
    </div>
  );
}
