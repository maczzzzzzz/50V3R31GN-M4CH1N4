...
const { writeClaw3dSettings } = require('./claw3d.cjs')
...
const APP_PORT = 3847
const HERMES_GATEWAY_URL = process.env.HERMES_GATEWAY_URL || 'http://127.0.0.1:8642/health'
const HERMES_DASHBOARD_URL = process.env.HERMES_DASHBOARD_URL || 'http://127.0.0.1:9119/api/status'
...
app.whenReady().then(async () => {
  try { writeClaw3dSettings(); } catch (e) { console.error('[desktop] claw3d setup failed:', e); }
