...
function readOverrides(): WorkspaceOverrides {
  try {
    ...
    console.log(`[gateway-debug] Read overrides from ${overridesPath()}: ${raw}`)
    const parsed = JSON.parse(raw) as WorkspaceOverrides
    return parsed && typeof parsed === 'object' ? parsed : {}
  } catch {
    console.log(`[gateway-debug] No overrides found at ${overridesPath()}`)
    return {}
  }
}
...
console.log(`[gateway-debug] Initial overrides:`, _initialOverrides)
console.log(`[gateway-debug] HERMES_API_URL env:`, process.env.HERMES_API_URL)
...
async function autoDetectGatewayUrl(): Promise<void> {
  ...
  console.warn(
    '[gateway] Could not reach Hermes gateway. ' +
      'Ensure Node D (100.120.225.12) is active and reachable via Tailscale.',
  )
}

async function autoDetectDashboardUrl(): Promise<void> {
  if (process.env.CLAUDE_DASHBOARD_URL || process.env.HERMES_DASHBOARD_URL) return

  const candidates = ['http://100.120.225.12:9119', 'http://127.0.0.1:9119']
  for (const candidate of candidates) {
    try {
