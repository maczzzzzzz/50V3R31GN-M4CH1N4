...
export function createTerminalSession(params: {
  ...
  // Sovereign Artery: Default to hermes chat --tui if available
  const hermesPath = resolve(homedir(), '.local/bin/hermes')
  const defaultCommand = existsSync(hermesPath) ? [hermesPath, 'chat', '--tui'] : [process.env.SHELL ?? defaultShell]
  ...
  const proc: ChildProcess = spawn(
    ...
    [PTY_HELPER, cwd, String(cols), String(rows), '--', ...command],
    {
      env: {
        ...
        COLUMNS: String(cols),
        LINES: String(rows),
        HERMES_GATEWAY_URL: CLAUDE_API,
        HERMES_API_KEY: 'sk-sovereign-mesh-proxy',
