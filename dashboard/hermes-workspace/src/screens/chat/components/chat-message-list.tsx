...
export function buildDisplayEntries(
  ...
  // Trailing tool-only assistant messages are NOT attached to the last text reply.
  // This allows getTrailingToolOnlyTurnSummary to detect them and show a summary instead.
...
export function getTrailingToolOnlyTurnSummary(
  messages: Array<ChatMessage>,
): { count: number; toolNames: string[]; hasFinalAssistantText: boolean } | null {
  const textAssistantEntries = messages.filter(
    (m) => m.role === 'assistant' && textFromMessage(m).trim().length > 0,
  )

  const lastAssistantWithText =
    textAssistantEntries[textAssistantEntries.length - 1]

  if (!lastAssistantWithText) {
    const toolOnly = messages.filter(
      (m) =>
        m.role === 'tool' ||
        m.role === 'toolResult' ||
        isAssistantToolCallOnlyMessage(m),
    )
    if (toolOnly.length === 0) return null

    const toolNames = new Set<string>()
    toolOnly.forEach((m) => {
      if (m.role === 'tool' || m.role === 'toolResult') {
        toolNames.add(m.toolName || 'terminal')
      } else {
        getToolCallsFromMessage(m).forEach((tc) => toolNames.add(tc.name))
      }
    })

    return {
      count: toolOnly.length,
      toolNames: Array.from(toolNames),
      hasFinalAssistantText: false,
    }
  }

  const lastAssistantWithTextIndex = messages.indexOf(lastAssistantWithText)
  const trailingMessages = messages.slice(lastAssistantWithTextIndex + 1)

  if (trailingMessages.length === 0) return null

  const toolOnlyMessages = trailingMessages.filter(
    (m) =>
      m.role === 'tool' ||
      m.role === 'toolResult' ||
      (m.role === 'assistant' && isAssistantToolCallOnlyMessage(m)),
  )

  if (toolOnlyMessages.length === 0) return null

  const toolNames = new Set<string>()
  toolOnlyMessages.forEach((m) => {
    if (m.role === 'tool' || m.role === 'toolResult') {
      toolNames.add(m.toolName || 'terminal')
    } else {
      getToolCallsFromMessage(m).forEach((tc) => toolNames.add(tc.name))
