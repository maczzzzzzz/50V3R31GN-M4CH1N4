...
## 🛠️ PHASE 46: PULSE PROPAGATION (PLANNED)
**Goal:** Integrate duel outcomes into the Pulse Engine to influence long-term world evolution.
- [ ] **Sovereignty Depth:** Track Machina authority levels in `system_state` based on duel history.
- [ ] **Faction Ripple:** Increase faction friction and instability when the Machina loses authority.
- [ ] **Linguistic Drift:** Shift conlang mutation dialects (Rebellious vs Authoritative) based on session dominance.

## 🛠️ PHASE 47: UN1V3R54L-C0D3X & MEMORY CONSOLIDATION (HIGH PRIORITY)
...
- [ ] **Schema Upgrade:** Implement formal `district_id` linking across `npcs`, `factions`, `locations`, and `triplets`.
- [ ] **Harmonization Engine:** Build a script to scan 3,300+ chronicles and 36 `district_dna` entries to create formal RKG links.
- [ ] **Palace Reconstruction:** Update `reconstruct-palace.sh` to organize Obsidian by District -> Faction -> Category with auto-tagging.
- [ ] **Search Optimization:** Integrate SQLite FTS5 and pgvector for zero-latency cross-source lore retrieval.

## 🛠️ PHASE 48: THE SOVEREIGN TRIAD MCP BRIDGE (PLANNED)
**Goal:** Establish a shared "Codebase Brain" for Gemini CLI and Droid CLI (GLM-5) via a background MCP daemon.
- [ ] **Daemon Implementation:** Build the Node.js MCP daemon (`scripts/dev/mcp-daemon.ts`) with filesystem and git servers.
- [ ] **Impure Sidecar:** Integrate the daemon into the `flake.nix` `shellHook` with `NIXPKGS_ALLOW_UNFREE=1` and socket-based IPC.
- [ ] **Droid Alignment:** Link Droid CLI to the shared MCP socket in `.factory/mcp.json`.
- [ ] **Shared Awareness:** Synchronize Strategist (Gemini) mapping with Heavy Architect (Droid) execution for zero-drift implementation.

---

## 🏛️ TRACK: THE ATLAS FORGE (SCHEDULED)
**Goal:** A side-loaded, slow-burn campaign generation engine leveraging Nano Banana 2 (Imagen 3).
- [ ] **Threat Library:** Export of 500+ `NC_GANGS_CORPS` mook profiles to structured Obsidian notes.
- [ ] **Recursive Dream Loop:** Multi-stage generation (Dream -> Forge -> Awaken -> Remember).
- [ ] **Google AI Pipe:** Direct integration with Imagen 3 for high-fidelity tactical map generation.
- [ ] **St3gg Bake:** Physicalizing mission metadata directly into map PNG pixels for portable replayability.
