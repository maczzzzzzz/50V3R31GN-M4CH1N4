Project Black-Ice v0.9.1 → ASP.GM-Agent Master Proposal, Implementation Plan & Design Specification
Document ID: ASP-GM-LOCAL-MASTER-v1.0
Date: April 1, 2026
Status: Architecture Locked & Fully Aligned with https://github.com/maczzzzzzz/-asp-gm-agent (current repo state, Phase 6 active)
1. Executive Summary
ASP.GM-Agent is a fully local, air-gapped, dual-node AI Game Master for Cyberpunk RED (Foundry VTT v12).
It uses 1-bit Bonsai 8B on the Nitro 5 for deterministic Rules Oracle duties and Mistral-Nemo 12B (FP8) on the main rig for rich narrative, linked by ClawLink.
...
Node A (Nitro 5 – headless): Rules Oracle
...
Rules/math always resolved by 1-bit Oracle before narrative
...
Main project: https://github.com/maczzzzzzz/-asp-gm-agent
