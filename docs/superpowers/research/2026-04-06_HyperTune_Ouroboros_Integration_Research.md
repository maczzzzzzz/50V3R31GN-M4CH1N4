...
1.  **Phase A (Dynamic Sampling):** Modify `SovereignCognitionClient.generateNarrative` to accept an `options` object `(temperature, top_p)` so the `HybridRoutingController` can alter sampling based on the event type (`resolve_attack` vs `oracle_roll`).
