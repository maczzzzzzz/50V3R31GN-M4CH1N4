...
**Goal:** Integrate Warp/Oz telemetry mining and Hermes v3.8.28-GOLD "Curator" capabilities to create a fully local, live coding partner experience via the Quaternary Mesh.

**Architecture:** A Rust-based `sovereign-warp-observer` sidecar on Node D streams Warp telemetry to MemPalace via MCP. The `HermesSingularity` orchestrates a structured `Triage -> Spec -> Implement -> Review` pipeline based on this telemetry. A new `gepa-curator` sidecar periodically prunes and grades the local skill library. All system prompts are updated to the `[IMPORTANT:]` standard to align with Hermes v3.8.28-GOLD.
...
### Task 3: Update System Prompts to Hermes v3.8.28-GOLD Standard
...
git commit -m "chore(prompts): migrate system markers to Hermes v3.8.28-GOLD [IMPORTANT:] standard"
```
