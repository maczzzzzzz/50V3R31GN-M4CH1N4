...
**Subject:** Collaborative World-Building & Persistent NPC Synapse
...
## 2. Integration into 50V3R31GN-M4CH1N4
...
## 3. Deep Persistent NPC Synapse (Architectural Foresight)
While Simulacrum is a copilot, we are leveraging its patterns to implement **Simulacrum Deep Synapse** in Phase 5+.

### 3.1 Synapse Tiering
1.  **Static Synapse (RAG):** NPC backstories and roles stored in `pgvector` (Node A).
2.  **Dynamic Synapse (Story Engine):** Immediate interaction history stored in the `StoryState` (Node B).
3.  **Cross-Session Synapse:** A persistent SQLite table mapping NPC IDs to "Synapse Summaries," allowing an NPC to remember past PC actions across months of real-time play.

---

## 4. Technical Constraints
- **Foundry Version:** Simulacrum is targeted at Foundry v13+. Since we are pinned to **v12** for stability, we must maintain a custom "Immersion Mesh" that backports the tool-calling patterns used by Simulacrum to the v12 WebSocket API.
