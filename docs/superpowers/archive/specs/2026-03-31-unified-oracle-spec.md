# Design Specification: Unified Strategic Strategic Oracle & Hybrid RKG (v3.2.19)
**Date:** March 31, 2026
**Subject:** Narrative Grounding & World State Persistence
**Status:** FINALIZED

## 1. Executive Summary
The Unified Strategic Strategic Oracle consolidates all mechanical and narrative truth onto Node B. By leveraging SQLite's `ATTACH DATABASE` capability, the system performs cross-database joins between the **Knowledge Graph (world.db)** and the **Session History (crush.db)**. This architecture eliminates "Narrative Drift" and fulfills the **100% Local Integrity** mandate.

## 2. Technical Architecture

### 2.1 The "Triple-SQLite" Data Plane
All data operations on Node B route through the `UnifiedStrategic Strategic OracleClient`, which manages the following local files:
...
2.  **Strategic Strategic Oracle Join:** Fetch current stats from `world.db` + last 5 chat messages from `crush.db`.
