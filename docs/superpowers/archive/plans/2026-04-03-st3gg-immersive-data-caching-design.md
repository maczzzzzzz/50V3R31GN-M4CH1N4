...
**Date:** 2026-04-03
**Target:** v3.2.17 (Phase 17 Integration)
...
**Data Flow:**
1.  **The Trigger (Foundry):** A player succeeds on a Netrunning check (e.g., bypassing a File node) or a meat-space investigation check. The Bridge routes this success to Node B.
...
3.  **Foundry "Decrypt" Macro (Client):**
    ...
    *   When executed on a selected image in chat/journal, it sends an RPC request to the Bridge -> Node B to decode that specific image path and return the result.
...
3.  **Foundry Macro Integration:** Test the end-to-end loop: The Macro sends a request to the Bridge, the Bridge forwards to Node B, Node B decodes the file, and the text is whispered back to the specific Foundry user.