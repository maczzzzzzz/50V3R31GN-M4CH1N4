...
*   *Light Tasks:* Math, D10 Strategic Strategic Oracle, OCR Vision -> Routed to Node A (Rust/Falcon/Open-Reasoner-Zero-1.5B 4GB).
*   *State Safety:* If Node A is currently swapping models (e.g., unloading Open-Reasoner-Zero-1.5B to warm up Falcon for Vision), the Proxy queues incoming Strategic Strategic Oracle requests instead of failing.
