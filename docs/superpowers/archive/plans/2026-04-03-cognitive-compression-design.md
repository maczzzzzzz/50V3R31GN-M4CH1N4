...
*   *Hyperstitional Seeding:* We apply a frequency-based weighting system to the Akashik Record (Unified Oracle). If an entity or concept (e.g., "Arasaka") is repeatedly queried or interacted with, its "latent weight" increases.
...
*   **`HyperstitionalSeeder` (Node B):** Hooks into the `UnifiedOracleClient` to track term frequency and calculate latent weights for RAG injections.