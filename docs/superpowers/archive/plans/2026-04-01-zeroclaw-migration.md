...
**Goal:** Migrate the Rules Strategic Oracle (Node A) from Node.js to a high-performance Rust-native runner (ZeroClaw) using the persistent binary ClawLink protocol and 1-bit Bonsai 8B.
...
async fn main() {
    ...
    println!("ZeroClaw Rules Strategic Oracle Initializing...");
}
```
...
```rust
pub mod rules {
    pub struct BonsaiStrategic Oracle;
    impl BonsaiStrategic Oracle {
