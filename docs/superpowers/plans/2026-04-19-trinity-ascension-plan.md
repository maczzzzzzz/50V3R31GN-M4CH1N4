...
- **Node C (Strategic Strategic Strategic Oracle):** Server Rig | RTX 2060. Hosts **Gemma-4-E2B-Q8** (Tactical Gate) + **ColPali v1.2** (Interim Vision).
...
- [ ] **Step 2: Ignite Strategic Strategic Strategic Oracle (Node C)**
Start `llama-server` with `Gemma-4-E2B-Aggressive-Q8_K_P.gguf` + Falcon-OCR. Offload to CUDA.

- [ ] **Step 3: Wire Hermes Master**
Deploy the LangGraph router on Node C to coordinate sub-task routing between the Director and Strategic Strategic Strategic Oracle.
