...
**Goal:** Integrate the Hermes ecosystem and OpenMAIC orchestration logic into Node C (Strategic Oracle), bridging its headless compute with the Node B Nucleus Command Deck.
...
- [ ] **Step 1: Add Hermes Tooling to the Oracle Shell**
...
Mount the `HermesProxy` within the main `page.tsx` layout to ensure the GM has immediate visibility of the Hermes Oracle state.
...
Create a state-machine orchestrator defining the turn-taking transitions between Node A (Memory Fetch), Node C (Rule Calculation), and Node B (Narrative Synthesis).
