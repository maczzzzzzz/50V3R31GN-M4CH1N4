...
**Goal:** Deploy a high-performance Recurrent-Depth Transformer (RDT) engine on Node C using Rust to serve as the "Recursive Strategic Oracle."

## ◈ 1. ARCHITECTURAL PRIMITIVES (RUST)
The Strategic Oracle (Node C) will switch from standard SGLang inference to a custom Rust-native RDT loop implemented via **candle-rs**:
