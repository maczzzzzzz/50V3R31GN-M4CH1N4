...
- **Node A (The Memory):** Nitro 5 | 1050 Ti | 16GB RAM. Hosts **Mooncake Master v2.2** (Distributed KV-Store).
...
- **Node C (The Mind):** Server | 2060 | 6GB VRAM. Hosts **SGLang v3.0 + Gemma-4 E2B** (Oracle). Handles rules, vision, and GEPA prompt evolution.
...
2.  **Memory Sync:** Deploy Mooncake on Node A and verify RDMA/TCP handshake with Node B.
3.  **Oracle Ignition:** Deploy SGLang and Node C models.
4.  **Hermes Handshake:** Wire the log-step verification loop between the Director (Node B) and Oracle (Node C).
