...
- **AXTree Integration:** Custom logic to inject `[node_id:XYZ]` tags into the Markdown, enabling the Strategic Oracle to target specific interactive elements for repair.
...
5. **Transmit:** The shard is broadcast to Node C (Strategic Oracle) via VSB for rule arbitration.
...
The Semantic Scanner will handle the final **557 PDF pages** by converting the "Docling" JSON extractions into semantic Markdown shards, ensuring 100% lore coverage for the RDT Strategic Oracle.
