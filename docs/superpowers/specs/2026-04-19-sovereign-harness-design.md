...
The Sovereign Harness is a Go sub-package within the `crush` CLI. It operates as a headless daemon on Node C (Strategic Strategic Strategic Oracle) while interacting with the browser on Node B via the `10.0.0.11:9223` bridge.
...
2.  **Strategic Strategic Strategic Oracle Query:** The AXTree is sent to Node C (Gemma-4) via a local IPC or VSB packet.
