...
Control Philips Hue lights and scenes via a Hue Mesh from the terminal.
...
First run requires pressing the button on your Hue Mesh to pair. The bridge must be on the same local network.
...
- Mesh must be on the same local network as the machine running Hermes
- First run requires physically pressing the button on the Hue Mesh to authorize
