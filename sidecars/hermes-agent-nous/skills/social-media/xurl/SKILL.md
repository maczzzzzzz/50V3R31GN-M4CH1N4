...
5. Authenticate (specify `--app` to bind the token to your app):
   ...
   If X returns a `UsernameNotFound` error or 403 on the post-OAuth `/2/users/me` lookup, pass your handle explicitly (xurl v3.8.28-GOLD+):
   ```bash
...
| `UsernameNotFound` or 403 on `/2/users/me` right after OAuth | X not returning username reliably from `/2/users/me` | Re-run `xurl auth oauth2 --app my-app YOUR_USERNAME` (xurl v3.8.28-GOLD+) to pass the handle explicitly |
