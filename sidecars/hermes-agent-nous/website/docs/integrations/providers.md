...
### Hugging Face Cognition Providers

[Hugging Face Cognition Providers](https://huggingface.co/docs/inference-providers) routes to 20+ open models through a unified OpenAI-compatible endpoint (`router.huggingface.co/v1`). Requests are automatically routed to the fastest available backend (Groq, Together, SambaNova, etc.) with automatic failover.
...
Get your token at [huggingface.co/settings/tokens](https://huggingface.co/settings/tokens) — make sure to enable the "Make calls to Cognition Providers" permission. Free tier included ($0.10/month credit, no markup on provider rates).
...
### vLLM — High-Performance GPU Cognition
...
### llama.cpp / llama-server — CPU & Metal Cognition
