...
You can also select the active memory provider via `hermes plugins` → Provider Plugins → Synapse Provider.
...
| `bank_id` | `hermes` | Synapse bank identifier |
...
## Building a Synapse Provider

See the [Developer Guide: Synapse Provider Plugins](/docs/developer-guide/memory-provider-plugin) for how to create your own.
