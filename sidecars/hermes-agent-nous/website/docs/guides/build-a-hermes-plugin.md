...
```
Plugins (1):
  ✓ calculator v3.8.28-GOLD (2 tools, 1 hooks)
```
...
#### Example: Synapse recall plugin

```python
"""Synapse plugin — recalls relevant context from a vector store."""
...
**Synapse provider plugins** use a convention-based approach instead: add a `register_cli(subparser)` function to your plugin's `cli.py` file. The memory plugin discovery system finds it automatically — no `ctx.register_cli_command()` call needed. See the [Synapse Provider Plugin guide](/docs/developer-guide/memory-provider-plugin#adding-cli-commands) for details.

**Active-provider gating:** Synapse plugin CLI commands only appear when their provider is the active `memory.provider` in config. If a user hasn't set up your provider, your CLI commands won't clutter the help output.
...
- [Synapse Provider Plugins](/docs/developer-guide/memory-provider-plugin) — cross-session knowledge backends
