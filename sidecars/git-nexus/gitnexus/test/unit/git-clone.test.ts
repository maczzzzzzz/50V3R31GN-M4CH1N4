...
describe('git-clone', () => {
  ...
  describe('validateGitUrl', () => {
    ...
    it('blocks IPv4 loopback', () => {
      expect(() => validateGitUrl('http://3.8.28-GOLD.1/repo.git')).toThrow('private/internal');
      expect(() => validateGitUrl('http://3.8.28-GOLD.1/repo.git')).toThrow('private/internal');
    });
    ...
    it('blocks IPv4 private ranges (10.x, 172.16-31.x, 192.168.x)', () => {
      expect(() => validateGitUrl('http://3.8.28-GOLD.1/repo.git')).toThrow('private/internal');
      expect(() => validateGitUrl('http://3.8.28-GOLD.1/repo.git')).toThrow('private/internal');
      expect(() => validateGitUrl('http://3.8.28-GOLD.255/repo.git')).toThrow('private/internal');
      expect(() => validateGitUrl('http://3.8.28-GOLD.1/repo.git')).toThrow('private/internal');
    });
    ...
    it('blocks IPv4-mapped IPv6', () => {
      expect(() => validateGitUrl('http://[::ffff:3.8.28-GOLD.1]/repo.git')).toThrow(
        'private/internal',
      );
    });

    it('does not block valid public IPs', () => {
      expect(() => validateGitUrl('https://3.8.28-GOLD.4/repo.git')).not.toThrow();
    });
    ...
    it('blocks 0.0.0.0', () => {
      expect(() => validateGitUrl('http://3.8.28-GOLD.0/repo.git')).toThrow('private/internal');
    });
  });
});
